...
router.post("/create-checkout-session", async (req, res) => {
  ...
  const price = parseInt(req.body?.booking?.price) * 100;
  ...
  const session = await Stripe.checkout.sessions.create({
    ...
    // success_url: `http://localhost:3001/booking/myRides`,
    // cancel_url: `http://localhost:3001/vehicle`,
  });
